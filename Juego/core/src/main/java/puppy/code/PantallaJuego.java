...
public class PantallaJuego implements Screen {
    ...
    private Sound soundVida;
    private Sound soundEscudo;
    private Sound soundVelocidad;
    private Sound soundDobleMisil;
    private int score;
    private int ronda;
    ...
    private Texture texturaDobleMisil;
    ...
    private ArrayList<Explosiones> explosiones = new ArrayList<>();
    ...
    public PantallaJuego(SpaceNavigation game, int ronda, int vidas, int score,
        ...
        soundVida = Gdx.audio.newSound(Gdx.files.internal("PowerUpVidaExtra.mp3"));
        soundEscudo = Gdx.audio.newSound(Gdx.files.internal("PowerUpEscudo.mp3"));
        soundVelocidad = Gdx.audio.newSound(Gdx.files.internal("PowerUpFlash.mp3"));
        soundDobleMisil = Gdx.audio.newSound(Gdx.files.internal("PowerUpMisilDoble.mp3"));
        ...
        texturaDobleMisil = new Texture(Gdx.files.internal("TorpedoX2.png"));
        texturaExplosion = new Texture(Gdx.files.internal("explosion.png"));
        ...
        float naveX = nave.getHitbox().x;
        float naveY = nave.getHitbox().y;
        ...
        for (int i = 0; i < cantAsteroides; i++) {
            ...
            do {
                ...
                for (Ball2 existente : balls1) {
                    float dxA = x - existente.getHitbox().x;
                    float dyA = y - existente.getHitbox().y;
    ...
    public void dibujaHUD() {
        ...
        game.getFont().draw(batch, "HighScore: " + HighScoreManager.getInstance().getHighScore(), 10, 30);
    }
    ...
    public void render(float delta) {
        ...
        if (!enPausa) {
            yFondo -= 100 * delta;
            if (yFondo <= -Gdx.graphics.getHeight()) {
                yFondo = 0;
            }
        }
        ...
        if (enPausa) {
            ...
            batch.begin();
            batch.draw(fondoTextura, 0, yFondo, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
            batch.draw(fondoTextura, 0, yFondo + Gdx.graphics.getHeight(), Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
        ...
        if (!dobleMisilActivado) {
            ...
            if (tiempoAcumulado >= 20f && itemDobleMisil == null) {
                ...
                itemDobleMisil = new PowerUpDobleMisil(xRand, Gdx.graphics.getHeight(), texturaDobleMisil, soundDobleMisil);
                tiempoAcumulado = 0f;
            }
        }
            ...
            for (int i = 0; i < balas.size(); i++) {
                ...
                for (int j = 0; j < balls1.size(); j++) {
                    if (b.checkCollision(balls1.get(j))) {
                        ...
                        float spawnX = minaDestruida.getHitbox().x;
                        float spawnY = minaDestruida.getHitbox().y;

                        explosiones.add(new Explosiones(texturaExplosion, spawnX, spawnY,
                            minaDestruida.getHitbox().width));

                        if (r.nextInt(100) < 30) {
                            int tipoPowerUp = r.nextInt(3);
                            switch (tipoPowerUp) {
                                case 0:
                                    powerUps.add(new PowerUpVidaExtra(spawnX, spawnY, texturaVidaExtra, soundVida));
                                    break;
                                case 1:
                                    powerUps.add(new PowerUpVelocidad(spawnX, spawnY, texturaVelocidad, soundVelocidad));
                                    break;
                                case 2:
                                    powerUps.add(new PowerUpEscudo(spawnX, spawnY, texturaEscudo, soundEscudo));
                                    break;
                            }
                        }
                        balls1.remove(j);
        ...
        if (itemDobleMisil != null) {
            ...
            if (nave.getHitbox().overlaps(itemDobleMisil.getHitbox())) {
                itemDobleMisil.activarPowerUp(nave);
        ...
        for (int i = 0; i < balls1.size(); i++) {
            ...
            if (nave.checkCollision(b)) {
                explosiones.add(new Explosiones(texturaExplosion, b.getHitbox().x, b.getHitbox().y,
                    b.getHitbox().width));
                balls1.remove(i);
                balls2.remove(i);
                i--;
        ...
        for (int i = explosiones.size() - 1; i >= 0; i--) {
            Explosiones e = explosiones.get(i);
            e.update(delta);
            if (e.isFinished()) {
    ...
    public void dispose() {
        ...
        this.texturaDobleMisil.dispose();
        this.texturaExplosion.dispose();

        this.soundVida.dispose();
        this.soundEscudo.dispose();
        this.soundVelocidad.dispose();
        this.soundDobleMisil.dispose();
    }
}
