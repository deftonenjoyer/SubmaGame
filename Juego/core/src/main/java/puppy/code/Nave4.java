...
public class Nave4 implements Actualizable {
    ...
    public void update(float delta) {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
            ...
            if (ahora - ultimoDisparo >= COOLDOWN_DISPARO) {
                disparar(); // ahora usa la estrategia
                ultimoDisparo = ahora;
            }
        }
    }
    ...
    private void disparar() {
        if (juego == null || estrategiaDisparo == null) return;
        estrategiaDisparo.disparar(this, juego);
    }

    public boolean checkCollision(Ball2 b) {
        if (!herido && b.getHitbox().overlaps(this.getHitbox())) {
            if (tieneEscudo) {
                tieneEscudo = false;
                sonidoHerido.play();
    ...
    public void activarDobleCohete(float duracion) {
        ...
        this.estrategiaDisparo = new DisparoDoble();
    }
